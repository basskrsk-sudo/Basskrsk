...
-- Журнал автоматических напоминаний о выплате. Уникальность не позволяет
...
CREATE TABLE IF NOT EXISTS payout_reminder_log (
  ...
  created_at     TEXT NOT NULL DEFAULT (datetime('now')),
  sent_at        TEXT,
  UNIQUE(recipient_role, recipient_id, payout_date, channel)
);

-- Журнал уведомлений о фактически выполненных выплатах. Запись привязана
-- к выплате, поэтому повторный запрос не сможет продублировать сообщение.
CREATE TABLE IF NOT EXISTS payout_notification_log (
  id             INTEGER PRIMARY KEY AUTOINCREMENT,
  recipient_role TEXT NOT NULL, -- 'partner' | 'owner'
  recipient_id   INTEGER NOT NULL,
  ...
  created_at     TEXT NOT NULL DEFAULT (datetime('now')),
  sent_at        TEXT,
  UNIQUE(recipient_role, payout_id, channel)
);
...
ensureColumn('max_login_tokens', 'role', "TEXT NOT NULL DEFAULT 'customer'");
ensureColumn('max_login_tokens', 'account_id', 'INTEGER');
...
-- календарную неделю хранится одна актуальная версия; повторное формирование
-- обновляет файлы и статус доставки, не засоряя список дублями.
