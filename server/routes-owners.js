...
function safeOwner(o) {
  const { password_hash, telegram_chat_id, max_chat_id, ...rest } = o;
...
function registerOwnerRoutes(router) {
  ...
  router.get('/api/salon-owners/:id/payout-preview', (req, res, ctx) => {
    const payload = requireAuth(['admin'])(req, res, ctx);
    if (!payload) return;
    const preview = previewOwnerPayout(Number(ctx.params.id));
    if (!preview) return sendJson(res, 404, { error: 'Владелец салона не найден' });
    if (preview.amount <= 0 || preview.orders_count <= 0) {
      return sendJson(res, 409, { error: 'У владельца сейчас нет невыплаченного вознаграждения' });
    }
  ...
  router.post('/api/salon-owners/:id/payouts', async (req, res, ctx) => {
    const payload = requireAuth(['admin'])(req, res, ctx);
    if (!payload) return;
    try {
      const ownerId = Number(ctx.params.id);
      const preview = previewOwnerPayout(ownerId);
      if (!preview) return sendJson(res, 404, { error: 'Владелец салона не найден' });
      ...
      if (
        ...
        preview.message !== expectedMessage
      ) {
      ...
      const payout = createOwnerPayout(ownerId, payload);
      let deliveries;
      try {
        deliveries = await sendOwnerPayoutNotification(ownerId, payout, expectedMessage);
      } catch (notificationError) {
        console.error('[owner-payout] Выплата записана, но уведомление не обработано:', notificationError);
