...
const server = http.createServer(async (req, res) => {
  ...
  // Лёгкий QR-лендинг: старые QR-коды вида /?ref=X&product=Y перенаправляем
  // на /p/X/Y, чтобы сканирование открывало компактную карточку товара
  // (public/p.html, ~12 КБ), а не всю витрину index.html.
  if (pathname === '/' && query.product && query.ref) {
