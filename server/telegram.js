...
// отчётов; повтор при сетевом тайм-ауте намеренно не делаем, чтобы не
// задвоить тяжёлое вложение, если Telegram его принял, но ответ потерялся.
async function sendTelegramDocument(filePath, caption) {
  if (!TG_TOKEN || !TG_CHAT_ID) {
    console.warn('Telegram не настроен — файл отчёта сохранён, но не отправлен');
    return { ok: false, skipped: true, error: 'TG_TOKEN или TG_CHAT_ID не задан' };
  }
  const controller = new AbortController();
  ...
  try {
    ...
    const response = await fetch(`https://api.telegram.org/bot${TG_TOKEN}/sendDocument`, {
      method: 'POST', body: form, signal: controller.signal,
    });
    const data = await response.json().catch(() => ({}));
    if (!response.ok) console.error('Ошибка отправки файла в Telegram:', data.description || response.status);
...
module.exports = { sendTelegram, sendToChat, sendTelegramDocument, buildOrderMessage };
