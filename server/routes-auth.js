...
function registerAuthRoutes(router) {
  ...
  router.post('/api/auth/login', async (req, res, ctx) => {
    ...
    const { password_hash, telegram_chat_id, max_chat_id, ...safeUser } = user;
  ...
  // Подключение MAX из уже открытого кабинета партнёра или владельца.
  // Как и в клиентском входе, бот принимает только контакт самого
  // пользователя; роль и account_id дополнительно связывают одноразовую
  // ссылку с конкретным авторизованным кабинетом.
  const maxAccountTables = {
    partner: 'partners',
    owner: 'salon_owners',
  };

  router.post('/api/max/connect/start', (req, res, ctx) => {
    const payload = requireAuth(Object.keys(maxAccountTables))(req, res, ctx);
    if (!payload) return;
    const { isConfigured } = require('./max-bot');
    if (!isConfigured()) return sendJson(res, 503, { error: 'MAX-бот пока не настроен на сервере' });
    const tableName = maxAccountTables[payload.role];
    const account = db.prepare(`SELECT id, phone, max_chat_id FROM ${tableName} WHERE id = ? AND active = 1`).get(payload.id);
    if (!account) return sendJson(res, 404, { error: 'Аккаунт не найден или отключён' });

    const token = crypto.randomBytes(24).toString('hex');
    const expiresAt = new Date(Date.now() + 10 * 60 * 1000).toISOString();
    db.prepare(`
      INSERT INTO max_login_tokens (token, phone, role, account_id, expires_at)
      VALUES (?, ?, ?, ?, ?)
    `).run(token, normalizePhoneDigits(account.phone), payload.role, payload.id, expiresAt);

    const botUsername = String(process.env.MAX_BOT_USERNAME).replace(/^@/, '');
    sendJson(res, 200, {
      token,
      deep_link: 'https://max.ru/' + botUsername + '?start=' + token,
      max_connected: !!account.max_chat_id,
    });
  });

  router.get('/api/max/connect/check', (req, res, ctx) => {
    const payload = requireAuth(Object.keys(maxAccountTables))(req, res, ctx);
    if (!payload) return;
    const token = String(ctx.query.token || '');
    if (!token) return sendJson(res, 400, { error: 'Не указан токен подключения' });
    ...
    const tableName = maxAccountTables[payload.role];
    db.prepare(`UPDATE ${tableName} SET max_chat_id = ? WHERE id = ?`).run(String(record.chat_id), payload.id);
    sendJson(res, 200, { verified: true, max_connected: true });
  });
  ...
  router.get('/api/partners/telegram/check', (req, res, ctx) => {
    ...
    const { password_hash: ph1, telegram_chat_id: tg1, max_chat_id: mx1, ...safePartnerUser } = partner;
    safePartnerUser.telegram_connected = !!tg1;
    safePartnerUser.max_connected = !!mx1;
  ...
  router.get('/api/customer/max/check', (req, res, ctx) => {
    const token = ctx.query.token;
    if (!token) return sendJson(res, 400, { error: 'Укажите token' });
    const record = db.prepare("SELECT * FROM max_login_tokens WHERE token = ? AND role = 'customer'").get(token);
