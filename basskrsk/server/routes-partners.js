...
function safePartner(p) {
  const { password_hash, telegram_chat_id, max_chat_id, ...rest } = p;
...
function registerPartnerRoutes(router) {
  ...
  // Перед фиксацией выплаты администратор видит точный текст и каналы,
  // в которые сообщение будет отправлено после подтверждения.
  router.get('/api/partners/:id/payout-preview', (req, res, ctx) => {
    const payload = requireAuth(['admin'])(req, res, ctx);
    if (!payload) return;
    const preview = previewPartnerPayout(Number(ctx.params.id));
    if (!preview) return sendJson(res, 404, { error: 'Партнёр не найден' });
    if (preview.amount <= 0 || preview.orders_count <= 0) {
      return sendJson(res, 409, { error: 'У партнёра нет невыплаченного вознаграждения' });
    }
  ...
  router.post('/api/partners/:id/payouts', async (req, res, ctx) => {
    const payload = requireAuth(['admin'])(req, res, ctx);
    if (!payload) return;
    try {
      const partnerId = Number(ctx.params.id);
      const preview = previewPartnerPayout(partnerId);
      if (!preview) return sendJson(res, 404, { error: 'Партнёр не найден' });
