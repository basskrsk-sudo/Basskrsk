...
# ImageMagick — для сжатия/изменения размера картинок товаров, загружаемых
# через админку/кабинет кладовщика (без него сервер не сможет обработать
# загруженные фото, будет ошибка при попытке создать товар с картинкой).
...
RUN (convert -size 10x10 xc:red /tmp/selftest.jpg && \
    ...
    echo "⚠️⚠️⚠️ ВНИМАНИЕ: ImageMagick не может обработать JPEG→WEBP в этой сборке Alpine (не хватает декодера или делегата cwebp) — загрузка фото товаров в админке работать НЕ БУДЕТ. Остальной сайт при этом продолжит работать нормально. ⚠️⚠️⚠️"
